...
def test_root():
    ...
    assert response.json() == {"message": "FraudShield AI Running"}
...
def test_fraud_distribution():
    response = client.get("/fraud-distribution")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) == 2
    assert data[0]["name"] == "Legitimate"
    assert data[1]["name"] == "Fraud"

def test_recent_transactions():
    response = client.get("/recent-transactions?limit=3")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) <= 3
...
def test_daily_trend():
    response = client.get("/daily-trend")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)